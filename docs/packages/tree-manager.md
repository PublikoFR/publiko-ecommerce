...
- **Import / export JSON** : boutons Export/Import dans le `headerEnd` slot de chaque `<x-filament::section>`, contextuels par arbre. Format versionné (`version: 1`). Import = transaction + `fixTree()` final côté collections, `updateOrCreate` par handle côté features (préserve l'ID existant).
  ...
  - **Aperçu de l'image dans la modale d'édition** : `collectionFormSchema(?int $collectionId)` préfixe un `Placeholder` « Image actuelle » quand on édite une catégorie (absent en création). L'URL passe par `pko_media_url($media, 'small')`, qui retombe sur l'original si la conversion n'est pas générée — les conversions partent en queue (`QUEUE_CONNECTION=redis`), donc elles sont absentes juste après un import tant qu'aucun worker ne tourne. Le champ `FileUpload` est relabellisé « Remplacer l'image » en édition.
