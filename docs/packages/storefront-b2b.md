...
- **Résolution du groupe par défaut — `Pko\CustomerAuth\Support\DefaultCustomerGroup`** (2026-07). Point d'entrée unique (`resolve()` / `matches()` / `handle()`) partagé par `RegisterProCustomer`, `ProAccess` et `CustomerGroupGuard`, qui faisaient chacun leur propre `where('handle', 'nouveau-client')`. La résolution est tolérante : handle exact, puis handle slugifié, puis nom slugifié.
  **Pourquoi** (incident dev 2026-07-28) : le champ `handle` de la resource Lunar est un `TextInput` libre. Le groupe « Nouveau client » recréé à la main depuis l'admin portait le handle `Nouveau client`, non slugifié → le lookup strict ne matchait rien. Double conséquence **silencieuse** (aucune exception, aucun log) : l'inscription n'attachait aucun groupe, et `ProAccess::denialReason()` refusait l'accès pro à **tous** les comptes. Le seeder créant bien `nouveau-client`, le bug était invisible en local et en test.
  Corrigé au passage : le fallback de `ProAccess` valait `'installateurs'` (handle inexistant) au lieu de `'nouveau-client'` → refus systématique si la config manquait.
- **Invariant handle = slug — `Pko\CustomerAuth\Observers\CustomerGroupHandleObserver`** : enregistré dans `CustomerAuthServiceProvider::boot()`, il slugifie tout `handle` à l'écriture (quel que soit le chemin : admin, seeder, tinker, import), dé-duplique par suffixe `-2`, `-3`… (la colonne est unique, deux noms distincts pouvant produire le même slug), et **verrouille** le handle du groupe par défaut — un ancien handle non slugifié est donc réaligné dès sa première sauvegarde. Couvert par `RegisterProCustomerTest` (rattachement malgré un handle dégradé, slugification, verrou du groupe par défaut).
- **Réparation des comptes orphelins** : `php artisan pko:customers:backfill-default-group [--dry-run]` rattache au groupe par défaut les clients qui n'y sont pas — comptes créés pendant que le lookup échouait, restés sans accès pro. Idempotente, ne détache jamais rien.
...
- **Raison sociale obligatoire** (`companyName` → `required`). Elle n'est préremplie automatiquement que si l'INSEE est activé (`INSEE_ENABLED=false` par défaut) : la laisser facultative produisait des `Customer.company_name` nuls, donc un champ « Raison sociale » vide au premier checkout. Verrouillé par `RegisterAutoLoginTest::test_company_name_is_required`.
- Après création : **auto-login inconditionnel** → `Auth::login()` + `session()->regenerate()` + `JustRegistered::flag()` + redirection vers **l'accueil `/`** (message de bienvenue invitant à vérifier l'e-mail). Le compte reste `pending` tant que l'e-mail n'est pas vérifié, mais le flag `JustRegistered` lui accorde un **accès complet** le temps de sa session (cf. § « semi-connexion » plus bas) — pas de rebond vers `/connexion`.

  **RÉGRESSION RÉCURRENTE — ne jamais reconditionner l'auto-login au statut SIRENE.** Le code a été plusieurs fois réécrit en `if ($result['sirene']->isActive())`, ce qui **désactive l'auto-login sur toute installation sans compte INSEE** : `RegisterProCustomer` a déjà rejeté le seul statut disqualifiant (`Status::Inactive` → `DomainException`), et le statut restant `Status::Pending` ne signifie pas « en attente de validation » mais « INSEE non consulté » — c'est le retour nominal quand `INSEE_ENABLED=false` (le défaut), quand la clé API est absente, en timeout ou en 5xx. C'est cohérent avec `ProAccess`, où le SIRET n'est pas un critère d'accès : seule la vérification e-mail l'est. Verrouillé par `RegisterAutoLoginTest::test_registration_logs_the_customer_in_when_insee_is_disabled`.

**Préremplissage de la première commande** (`App\Livewire\CheckoutPage::prefilledAddress()`) :
- Ordre de résolution de l'adresse : colonnes **`pko_street` / `pko_postcode` / `pko_city`** (ce que le client a saisi à l'inscription) **puis** `meta.sirene_address` en repli. Même piège que ci-dessus : `meta.sirene_address` n'est alimenté que si l'INSEE est activé, s'appuyer uniquement dessus laissait toute l'adresse vide au checkout.
- `company_name` vient de `Customer.company_name` (d'où la raison sociale rendue obligatoire à l'inscription). Aucun repli sur `User.name` : y injecter un nom de personne serait pire qu'un champ vide.
- Verrouillé par `tests/Feature/CheckoutPrefillTest.php`.
...
- La route de vérif fonctionne **sans session préalable** (clic depuis n'importe quel appareil) : valide signature + hash e-mail, `markEmailAsVerified()`, **promeut le(s) customer(s) `pending` → `active`** (inconditionnellement — plus de condition `sirene_status`), puis auto-login.
- `ProAccess::denialReason()` : un compte `pending` reçoit toujours le message « Confirmez votre adresse e-mail » (et `LoginPage` renvoie le lien). **Plus de gate `sirene_status`** dans `denialReason` : le SIRET ne bloque pas l'accès. Un compte à SIRET `pending` mais e-mail vérifié est donc pleinement actif. Verrouillé par `ProAccessRedirectTest::test_verified_account_is_active_even_when_siret_is_pending` + `RegisterProCustomerTest::test_verification_email_active_le_compte_meme_si_siret_pending`.
