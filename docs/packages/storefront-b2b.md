...
- **`pko_status` = `pending` à la création (toujours)** : le compte ne devient `active` qu'à la **vérification de l'e-mail** (route `verification.verify`). **C'est le SEUL critère d'activation** — le SIRET est validé à l'**inscription** (le compte n'est pas créé si le SIRET est mauvais), il ne rejoue **aucun** rôle à l'activation. En particulier, un `sirene_status='pending'` (INSEE désactivé ou indisponible) **n'empêche pas** l'activation : c'est le clic du client sur le lien e-mail qui active, jamais une validation admin. Voir §15.5 vérification e-mail.
