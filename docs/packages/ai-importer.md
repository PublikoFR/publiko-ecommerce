...
> **⚠️ Gotcha — modal « Configurer » systématiquement vide (corrigé 2026-06).** `fillForm`/`modalHeading` doivent lire l'état via **`getRawItemState($uuid)`**, jamais `getItemState($uuid)`. Ce dernier reconstruit l'état depuis les **composants enfants** du repeater de mapping ; or `actions`/`col`/`sheet`/`default` n'y sont pas (édités uniquement dans le modal) → ils sont écartés et le modal s'ouvre vide, alors même que le résumé de ligne (qui lit `$get('actions')` en raw) les affiche.
...
**Classes support :** `Pko\AiImporter\Support\ProductFieldCatalog` (champs produit canoniques Lunar + `LEGACY_LABELS` pour les libellés amis des clés PS conservées), `Pko\AiImporter\Support\ActionPalette` (palette catégorisée + labels FR, source unique pour le Select et le résumé) et `Pko\AiImporter\Support\PipelineSummary` (rendu serveur du résumé blueprint, dépliage `condition`). Branding neutre (aucun nom client). Garde-fous : `PipelineSummaryTest` (rendu + échappement) et `EditImporterConfigTest` (smoke test du rendu de la page Edit blueprint sur une config PS réelle).
