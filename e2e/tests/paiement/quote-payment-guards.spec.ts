...
test.describe('Paiement devis — gardes serveur', () => {
  test('le lien de paiement non signé est rejeté (403)', async ({ request }) => {
    const res = await request.get(`/paiement-devis/${SEEDED_ORDER_ID}`);
    expect(res.status()).toBe(403);
  });

  test('un lien de paiement avec signature invalide est rejeté (403)', async ({ request }) => {
    const res = await request.get(`/paiement-devis/${SEEDED_ORDER_ID}?signature=deadbeef&transport_cents=1000`);
    expect(res.status()).toBe(403);
  });

  test('la confirmation sans paiement en cours répond 410', async ({ request }) => {
    const res = await request.get(`/paiement-devis/${SEEDED_ORDER_ID}/confirmation`);
    expect(res.status()).toBe(410);
  });

  test('une commande inexistante répond 404', async ({ request }) => {
    const show = await request.get(`/paiement-devis/${UNKNOWN_ORDER_ID}`);
    expect(show.status()).toBe(404);

    const confirm = await request.get(`/paiement-devis/${UNKNOWN_ORDER_ID}/confirmation`);
