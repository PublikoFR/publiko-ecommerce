# Choix techniques — le storefront (back-office)
...
> **Contexte global** : remplacement du back-office PrestaShop 8 de le storefront (distributeur B2B matériaux de construction, domotique, portails, volets, automatismes) par un back-office Laravel + Lunar. Phase 1 = back-office pur, phases ultérieures = front, modules métier, intégrations externes.
...
## 7.quater Taxonomie produit — Product Types / Attributs Lunar vs Caractéristiques custom
...
## 7.sexies AI Core — providers LLM universels
...
## 8.quater Sticky footer admin — fix `min-h`
...
## 8.quinquies Global search admin — produits
...
## 13.ter Système de médias unifié — `pko_mediables`
...
## 13.quater Vidéos produit — `pko/product-videos`
...
## 13.quinquies Documents téléchargeables — `pko/product-documents`
...
## 13.sexies Page builder — `pko/page-builder`
...
## 13.septies CMS unifié multi-post-type
...
## 13.octies Brand pages via builder universel
...
## 13.nonies Filtre à facettes réactif (storefront)
...
## 13.duodecies Architecture packages OSS-ready — path repositories
...
_2026-04-17 — Admin CMS storefront (§15.12) : groupe Filament « Storefront » avec 7 resources (Slides, Tiles, Offers, Posts, Pages, Newsletter, Stores) + page Paramètres (contact/bannière/social/USPs/collection vedette). Table `pko_storefront_settings` key/value + modèle `Setting` avec cache 1h. Surcharge automatique de `config('storefront.*')` au boot. Cart drawer storefront (side-panel qui s'ouvre à l'add-to-cart)._

_2026-04-17 — Frontoffice B2B pro complet (§15) : 7 nouveaux packages (storefront + customer-auth + account + purchase-lists + quick-order + storefront-cms + store-locator), design system bleu pro, price gating strict (Connectez-vous pour voir vos prix), inscription auto + vérif SIRET INSEE V3, mega-menu cache 1h, catalogue faceté via catalog-features, home CMS (slides + tiles + offers + posts + pages), magasins Leaflet, autocomplete header DB. Compte pro de test : `thierry.leroy@example.test` / `testing123`._

_2026-04-17 — Admin dev credentials stabilisés via `PkoAdminUserSeeder` idempotent (§6.1) : `admin@example.fr` / `testing123` toujours restaurés par `make fresh`/`make install`. Overridable via `ADMIN_EMAIL` / `ADMIN_PASSWORD`._

_2026-04-17 — Système de médias unifié `pko_mediables` (§13.ter) : table pivot polymorphique universelle (media_id + mediable_type/id + mediagroup + position) remplaçant l'usage natif Lunar/Spatie côté admin. Trait `HasMediaAttachments`, champ Filament `MediaPicker` single/multi, modal Livewire globale réutilisable. 3 subclasses Lunar (`PkoProductResource`, `PkoCollectionResource`, `PkoBrandResource`) via reflection swap retirent les sous-pages/tabs media natifs. Modèles CMS (Post/Page/HomeSlide/HomeTile/HomeOffer) migrés, colonnes `cover_url`/`image_url` droppées après best-effort match._
