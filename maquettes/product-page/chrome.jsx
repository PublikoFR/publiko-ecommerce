/* Shared icons — thin stroke, lucide-style */
const Icon = ({ name, size = 16, className = '' }) => {
  const paths = {
    dashboard: 'M3 13h8V3H3v10zm0 8h8v-6H3v6zm10 0h8V11h-8v10zm0-18v6h8V3h-8z',
  };
  const svgMap = {
    tag: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20.59 13.41L13.42 20.58a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg>,
    ...
    feature: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01z"/></svg>,
    ...
    slides: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="4" width="20" height="14" rx="2"/><line x1="10" y1="20" x2="14" y2="20"/></svg>,
    ...
    offer: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>,
    ...
    store: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l1-5h16l1 5"/><path d="M5 9v11a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V9"/><path d="M3 9a3 3 0 0 0 6 0 3 3 0 0 0 6 0 3 3 0 0 0 6 0"/></svg>,
    ...
    settings: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.6 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>,
    ...
    percent: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="19" y1="5" x2="5" y2="19"/><circle cx="6.5" cy="6.5" r="2.5"/><circle cx="17.5" cy="17.5" r="2.5"/></svg>,
    ...
    sparkle: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3l2.09 5.26L20 9l-5 4 1.5 6-4.5-3-4.5 3L9 13l-5-4 5.91-.74L12 3z"/></svg>,
    ...
    history: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M1 4v6h6"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/><polyline points="12 7 12 12 15 14"/></svg>,
    ...
    underline: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M6 3v7a6 6 0 0 0 6 6 6 6 0 0 0 6-6V3"/><line x1="4" y1="21" x2="20" y2="21"/></svg>,
  };
  return <span className={`icon ${className}`} style={{ display: 'inline-flex', width: size, height: size }}>{svgMap[name]}</span>;
};
...
const Sidebar = () => {
  const items = [
    ...
    { section: 'Catalogue', items: [
      { icon: 'tag', label: 'Produits', active: true },
      { icon: 'bookmark', label: 'Marques' },
      { icon: 'grid', label: 'Catégories' },
      { icon: 'feature', label: 'Caractéristiques' },
    ]},
    { section: 'Storefront', items: [
      { icon: 'image', label: 'Médiathèque' },
      { icon: 'slides', label: 'Slides accueil' },
      { icon: 'layout', label: 'Tuiles accueil' },
      { icon: 'offer', label: 'Offres du moment' },
      { icon: 'news', label: 'Actualités' },
      { icon: 'page', label: 'Pages CMS' },
      { icon: 'store', label: 'Magasins' },
      { icon: 'mail', label: 'Abonnés newsletter' },
      { icon: 'settings', label: 'Paramètres' },
    ]},
    { section: 'Commandes', items: [
      { icon: 'cart', label: 'Commandes', badge: '1' },
      { icon: 'users', label: 'Clients' },
      { icon: 'percent', label: 'Réductions' },
    ]},
    { section: 'Marketing', items: [
      { icon: 'trophy', label: 'Paliers De Fidélité' },
      { icon: 'gift', label: 'Cadeaux Débloqués' },
    ]},
  ];
  return (
    <aside className="sidebar">
      <div className="sidebar-logo">
        ...
        <button className="sidebar-collapse" aria-label="Replier"><Icon name="chevronLeft" size={16}/></button>
      </div>
      {items.map((group, gi) => (
        <div key={gi}>
          {group.section && <div className="sidebar-section">{group.section}</div>}
          {group.items.map((it, i) => (
            <a key={i} className={`sidebar-item ${it.active ? 'active' : ''}`} href="#" onClick={e => e.preventDefault()}>
  ...
  <div className="breadcrumb">
    {items.map((it, i) => (
      <React.Fragment key={i}>
        ...
        {it.href ? <a href="#" onClick={e => e.preventDefault()}>{it.label}</a> : <span className="current">{it.label}</span>}
      </React.Fragment>
    ))}
  </div>
);
...
/* Field helpers */
const Field = ({ label, required, help, error, children, charCounter }) => (
  <div className="field">
    {label && (
      <label>
        {label}{required && <span className="req">*</span>}
        {charCounter && <span className="grow"></span>}
        {charCounter}
      </label>
    )}
    {children}
...
Object.assign(window, { Icon, Sidebar, Topbar, Breadcrumb, Field, Switch });
