...
4. **Admin** : resource Filament `CarrierShipmentResource` (groupe **Expédition**) liste les envois. Action "Relancer" visible uniquement pour `origin=weklo`. Les colonnes transporteur / service / statut d'envoi affichent un libellé FR via `CarrierDisplayLabel` (le slug reste la valeur stockée).
