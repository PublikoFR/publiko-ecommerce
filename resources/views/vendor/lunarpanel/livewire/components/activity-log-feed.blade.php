{{--
    Surcharge de la vue Lunar (vendor/lunarphp/lunar/resources/views/livewire/components/activity-log-feed.blade.php).
    Écarts : pas de marge latérale ni haute (le conteneur, une section Filament, les porte déjà) ;
    bouton « Ajouter un commentaire » dans le flux (il était en absolute et chevauchait la
    première date) ; dates et heures au format français.
--}}
<div class="pb-2 scroll-mt-32" id="lunar-panel-timeline">
                ...
                @foreach ($this->activityLog as $log)
                    <li class="relative py-8 ml-5">
                        <p class="ml-8 font-bold text-gray-950 dark:text-gray-300">
                            {{ ucfirst($log['date']->locale('fr')->translatedFormat('l j F Y')) }}
                        </p>
