...
@if ($logo)
    ...
    <img src="{{ $logo }}" alt="{{ $name }}" {{ $attributes->merge(['class' => 'h-14 w-auto']) }} />
@else
    {{-- Logo textuel de repli (Design System : forest + lime, formes arrondies) --}}
    <svg {{ $attributes->merge(['class' => 'h-12 w-auto', 'viewBox' => '0 0 220 44', 'fill' => 'none', 'xmlns' => 'http://www.w3.org/2000/svg']) }} aria-label="{{ $name }}">
